...
import { pointX, pointY, CHART_BOTTOM, DRAW_DURATION, TEXT_COLOR, DEFAULT_ACCENT } from "./config";

export const PriceLine: React.FC<{ content: PriceChartContent; maxValue: number }> = ({
  content,
  maxValue,
}) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  const color = content.accentColor || DEFAULT_ACCENT;
  const prefix = content.unitPrefix ?? "";

  const points = content.points.map((p, i) => ({
    x: pointX(i, content.points.length),
    y: pointY(p.value, maxValue),
    value: p.value,
  }));
  ...
  const totalLength = segmentLengths.reduce((a, b) => a + b, 0) || 1;
