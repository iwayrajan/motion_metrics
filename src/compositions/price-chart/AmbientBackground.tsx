...
import { BG_DARK, BG_GLOW, DEFAULT_ACCENT } from "./config";
...
// Mobile profile: "layer motion, don't single-task it" — persistent background motion
// so nothing reads as frozen while waiting for the next beat.
export const AmbientBackground: React.FC<{ accentColor?: string }> = ({ accentColor }) => {
  const frame = useCurrentFrame();
  const pulse = interpolate(frame % 90, [0, 45, 90], [0.5, 0.8, 0.5]);
  const color = accentColor || DEFAULT_ACCENT;
  const particleColors = [color, "#FFFFFF"];
